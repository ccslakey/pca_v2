...
interface Props {
  player: { isBatter: boolean; isPitcher: boolean; seasons: { length: number } };
  war:       number;
  peak:      Peak | null;
  careerHR:  number;
  careerSO:  number;
...
export function StatGrid({ player, war, peak, careerHR, careerSO, careerAVG, careerOPS, careerERA }: Props) {
    ...
    <div className="stat-grid">
      ...
      <div className="stat-block">
        ...
        <div className="stat-sub">
          {player.seasons.length > 0 ? (careerSO / player.seasons.length).toFixed(0) : '—'} / yr
        </div>
      </div>
    </div>
  );
