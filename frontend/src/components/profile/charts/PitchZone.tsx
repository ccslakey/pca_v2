...
const LAYERS: { threshold: number; rgb: RGB; blur: number; opacity: number }[] =
  [
    ...
    { threshold: 0.82, rgb: [175, 28, 28], blur: 2, opacity: 0.9 },
  ];
...
function toSvgX(ft: number) {
  return ((ft - X_MIN) / (X_MAX - X_MIN)) * WIDTH;
}
function toSvgZ(ft: number) {
  ...
  return (
    <div className="pitch-zone-wrap">
          ...
          <g clipPath={`url(#${clipId})`}>
            {hasData &&
              LAYERS.map((layer, i) => {
                ...
                const cells = [...grid.entries()].filter(
                  ([, { count, total }]) =>
      ...
      <div className="pitch-zone-legend">
        <span>Cold</span>
        <div
          className="pitch-zone-gradient"
          style={{
            background: `linear-gradient(to right, ${LAYERS.map(
              (l, i) =>
