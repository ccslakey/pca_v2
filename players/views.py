...
class PlayerViewSet(viewsets.ReadOnlyModelViewSet[Player]):
    ...
    def similar(self, request: Request, pk: str | None = None) -> Response:
        ...
        player_bat_war: float = (
            BattingSeason.objects.filter(player=player).aggregate(s=Sum("war"))["s"]
            or 0.0
        )
        player_pit_war: float = (
            PitchingSeason.objects.filter(player=player).aggregate(s=Sum("war"))["s"]
            or 0.0
        )
        ...
        for p in Player.objects.exclude(bbref_id=player.bbref_id).only(
            ...
            p_war = (bat_wars.get(p.bbref_id, 0.0) or 0.0) + (
                pit_wars.get(p.bbref_id, 0.0) or 0.0
            )
            if abs(p_war) < 1.0:
                continue
